...
public class PigmentCommand extends ActualHandle {
	...
	public boolean handle(MapPainting plugin, String prefix, CommandSender sender, String[] arguments) {
		...
		if (Pcolor.color != null) {
			...
			plugin.m_paletteManager.dye.setColor(item, Pcolor.color);
			PotionMeta meta = (PotionMeta) item.getItemMeta();
			Color awtColor = Pcolor.color;
			String paintname = "";
			if (!basic) {
				paintname = "§l§c" + awtColor.getRed() + " §a" + awtColor.getGreen() + " §9" + awtColor.getBlue();
			} else {
				String firstletter = arguments[0].substring(0,1).toUpperCase();
				String remainder = arguments[0].substring(1);
				paintname = "§f" + firstletter + remainder;
			}
			meta.setDisplayName("§6Paint: " + paintname);
			...
			item = new ItemStack(Material.SHEARS); // transparency handling
		}
